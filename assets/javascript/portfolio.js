...
$(document).ready(() => {
  const links = [
    {
      name: 'Discord',
      link: 'https://discord.gg/fKhd2x3qCm',
    },
    {
      name: 'Ratelimit.club',
      link: 'https://ratelimit.club',
    },
    {
      name: 'Github',
      link: 'https://github.com/IntelligenceHostingLLC/',
    },
    {
      name: 'Ohiocloud',
      link: 'https://ohiocloud.net/',
    },
    {
      name: 'Meeks Zombies',
      link: 'https://discord.gg/27Xr6ZRSFy',
    },
  ];
  ...
  app.titleChanger(['Took Her to the O', 'DDos Notifications', 'Mitigation Partner', 'Imperva Enterprise', 'No snitching bitch', 'Filtering Attacks all Day', 'Advanced Monitoring', ]);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>ratelimit.club $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
